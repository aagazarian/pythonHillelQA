from ArpineAgazaryan.pythonHillelQA.homework_13.task_1.passenger import \
    Passenger
from ArpineAgazaryan.pythonHillelQA.homework_13.task_2.train import Train
from ArpineAgazaryan.pythonHillelQA.homework_13.task_2.train_car import \
    TrainCar

if __name__ == "__main__":
    train = Train()

    # Well not the best practice and too implicit.
    # Action named like add so should not return anything
    # never return reference on object to prevent side effects
    train.add_train_car(
        TrainCar(10)
    ...
    for train_car in train.train_cars:
        print(
            f"TrainCar length (passenger amount) in train_car Number: '{train_car.self_number}' is '{train_car.len()}'"
        )
